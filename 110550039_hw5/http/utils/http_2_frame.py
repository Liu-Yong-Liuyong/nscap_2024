...
class Frame:
    ...
    def to_bytes(self):
        ...
        length_type = (self.length<<8) | self.type
        r_stream_id = (self.r << 31) | self.stream_id
        bytes = struct.pack("!LBL{}s".format(self.length),
                                  length_type, 
                                  self.flags,
                                  r_stream_id,
                                  self.payload)
...
def bytes_to_frames(data):
    ...
    while len(data) > 0:
        ...
        # Unpack the length and type from the frame header
        (length_type,) = struct.unpack("!L", data[:4])
        length = length_type >> 8
        ...
        if len(data) >= length + 9:
            ...
            frame_header = data[3:9]
            payload = data[9:9 + length]
            type, flags, r_stream_id = struct.unpack("!BBL", frame_header)
