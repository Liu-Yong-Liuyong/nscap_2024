...
def parse_response(client_socket, stream):
    ...
    version, status = lines[0].decode().split(' ', 1)
    ...
    for line in lines[1:]:
        if line:
            split_line = line.decode().split(': ', 1)  # Split only once to handle values containing ':'
            if len(split_line) == 2:  # Ensure there are exactly two elements after splitting
                key, value = split_line
                headers[key.lower()] = value.strip()  # Strip whitespace from key and value
    ...
    if 'content-length' in headers:
        ...
        if response.body_length >= content_length:
            response.complete = True
...
def parse_request(client_socket):
    ...
    # Hint: Bytes should be decoded using decode() to obtain a string.
    method, resource, version = lines[0].decode().split(' ')
    ...
    if 'content-length' in headers:
        ...
        content_length = int(headers['content-length'])
        if request.body_length >= content_length:
            request.complete = True
